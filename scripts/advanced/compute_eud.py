...
if compute_ind_shares:
    ...
    ind_shares['AVIATION_LONG_HAUL'] = 0
    ind_shares['SHIPPING'] = 0
...
av_short_haul = av_growth_mul.copy().mul(av_2019['Intra-EU aviation demand [Gpkm](Eurostat 2019)'], axis=0)
av_long_haul = av_growth_mul.copy().mul(av_2019['Extra-EU aviation demand [Gpkm](Eurostat 2019)'], axis=0)
...
for r in regions:
    ...
    if r2 == 'UK':
        ...
        euref_transport.loc['SHIPPING', :] = euref_transport.loc['MOBILITY_FREIGHT', :]\
            .mul(all_eud.loc[(years, 'IE', 'SHIPPING'), 'TRANSPORTATION'].values / all_eud.loc[(years, 'IE', 'MOBILITY_FREIGHT'), 'TRANSPORTATION'].values)
        # add aviation
        euref_transport.loc['MOBILITY_PASSENGER', :] += (-euref_transport.loc['Intra-EU aviation', :]
                                                         + av_short_haul.loc[r, :])
        euref_transport.loc['AVIATION_LONG_HAUL', :] = av_long_haul.loc[r, :]
    else:
        ...
        euref_transport.loc['SHIPPING', :] = euref_transport.loc['TOTAL_FREIGHT_ENERGY', :]\
                                                              .div(euref_transport.loc['TOTAL_FREIGHT_INTENSITY', :])\
                                                          - euref_transport.loc['MOBILITY_FREIGHT', :]
        euref_transport.loc['SHIPPING', :] = euref_transport.loc['SHIPPING', :]\
            .mask(euref_transport.loc['SHIPPING', :] < 0.1, 0)
    ...
    all_eud.loc[(years, r, 'AVIATION_LONG_HAUL'), 'TRANSPORTATION'] =\
        euref_transport.loc['AVIATION_LONG_HAUL', years].values
    all_eud.loc[(years, r, 'SHIPPING'), 'TRANSPORTATION'] =\
        euref_transport.loc['SHIPPING', years].values

# for CH and NO
all_eud.loc[(years, 'CH', 'AVIATION_LONG_HAUL'), :] = 0
all_eud.loc[(years, 'NO', 'AVIATION_LONG_HAUL'), :] = 0
all_eud.loc[(years, 'CH', 'AVIATION_LONG_HAUL'), 'TRANSPORTATION'] = av_long_haul.loc['CH', years].values
all_eud.loc[(years, 'NO', 'AVIATION_LONG_HAUL'), 'TRANSPORTATION'] = av_long_haul.loc['NO', years].values


""" Computing share_short_haul_flight"""
mob_pass_df = all_eud.loc[(years, av_short_haul.index, 'MOBILITY_PASSENGER'), 'TRANSPORTATION'].droplevel(level=2, axis=0)\
    .reset_index().pivot(index='level_1', columns='level_0').droplevel(level=0, axis=1)
share_short_haul_flight = av_short_haul.div(mob_pass_df)
...
if save_results:
    ...
    share_short_haul_flight.to_csv(project_path / 'Data' / 'exogenous_data' / 'regions' / 'share_short_haul_flight.csv',
                                   sep=CSV_SEPARATOR)
    ...
    for y in save_years:
        ...
        # saving ned shares short_haul_flight shares
        for r in ned_shares.index:
            ...
            misc['share_short_haul_flights_min'] = share_short_haul_flight.loc[r, y]
            misc['share_short_haul_flights_max'] = share_short_haul_flight.loc[r, y] + 1e-4
