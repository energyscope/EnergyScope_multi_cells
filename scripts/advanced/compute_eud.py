...
from esmc.common import eu33_country_code_iso3166_alpha2, eu33_full_names, eu27_country_code, CSV_SEPARATOR

full_2_code = dict(zip(eu33_full_names, eu33_country_code_iso3166_alpha2))

# TODO what about CH, NO, ... ?? (for CH there are data in eucalc, but data agric seems weird)

# case
compute_ind_shares = True
save_ind_shares = True
save_results = False
...
eui_names = ['ELECTRICITY', 'HEAT_HIGH_T', 'HEAT_LOW_T_SH', 'HEAT_LOW_T_HW',
             'PROCESS_COOLING', 'SPACE_COOLING', 'MOBILITY_PASSENGER', 'MOBILITY_FREIGHT', 'NON_ENERGY'] #end-uses inputs
eui_categories = ['Electricity', 'Heat', 'Heat', 'Heat', 'Cold', 'Cold', 'Mobility', 'Mobility', 'Non-energy']
eui_subcat = ['Electricity', 'High temperature', 'Space heating', 'Hot water',
              'Process cooling', 'Space cooling', 'Passenger', 'Freight', 'Non-energy']
...
for r in regions:
    ...
    if r2 == 'UK':
        ...
        euref_transport = euref_b.loc[['MOBILITY_PASSENGER', 'MOBILITY_FREIGHT']]
    else:
        ...
        euref_transport = pd.read_excel(data_path / euref_file, sheet_name=(r2 + '_B'), header=1, index_col=0) \
            .loc[['Passenger transport activity (Gpkm)', 'Freight transport activity (Gtkm)'], years] \
            .rename(index={'Passenger transport activity (Gpkm)': 'MOBILITY_PASSENGER',
                           'Freight transport activity (Gtkm)': 'MOBILITY_FREIGHT'})
    ...
    all_eud.loc[(years, r, 'MOBILITY_FREIGHT'), 'TRANSPORTATION'] = euref_transport.loc['MOBILITY_FREIGHT', years].values
...
if save_results:
    ...
    for y in years:
        for r in regions:
            ...
            my_df['Units'] = ['[GWh]', '[GWh]', '[GWh]', '[GWh]', '[GWh]', '[GWh]', '[Mpkm]', '[Mtkm]', '[GWh]']
